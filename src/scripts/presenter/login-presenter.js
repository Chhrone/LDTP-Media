...
class LoginPage {
  ...
  async afterRender() {
    ...
    loginForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        // Update navigation UI to show nav links and user button after successful login
  ...
  async _startLoadingAnimation() {
    // Always start with "Logging in..."
    this._view.updateLoadingText('Logging in...');
    await sleep(1500);

    // Remove "Logging in..." from the array to avoid duplication
